...
export class TripsService {
  ...
  /** Returns summary columns only (matches the legacy fetchTrips()). */
  ...
  async create(trip: Trip): Promise<Trip> {
    const inserted = await this.prisma.trip.create({
      data: {
        title: trip.title,
  ...
  /**
   * Update everything EXCEPT todos. Preserves the legacy contract: TripEditor won't
   * overwrite a newly-created todo that arrived after the user opened the editor
   * — todos go through patchTripTodos.
   */
  async update(id: string, partial: Partial<Trip>): Promise<Trip> {
    ...
    if (rest.dailyItineraries !== undefined)
      update.dailyItineraries = rest.dailyItineraries as object;
    ...
    try {
      const row = await this.prisma.trip.update({ where: { id }, data: update });
      return rowToTrip(recordToRow(row));
    } catch (err) {
      if (isNotFound(err)) throw HttpError.notFound(`Trip ${id} not found`);
      throw err;
    }
  }
  ...
  /**
   * Delete. Reminder rows cascade via FK, but we clear them first inside the same
   * tx so the route layer's post-commit queue cancellation sees a consistent set.
   */
   ...
   * snapshot (regression we saw in the old repo).
   */
  patchTodos(
    tripId: string,
