...
export class WeatherService {
  ...
  async fetchBundle(params: {
    ...
    const apiKey = this.config.OPENWEATHER_API_KEY;
    ...
    const cached = await this.redis.readJson<WeatherBundle>(cacheKey);
    ...
    await this.redis.writeJson(cacheKey, bundle, CACHE_TTL_SECONDS);
    return bundle;
  }

  async geocode(q: string, limit: number): Promise<GeoCityHit[]> {
    const apiKey = this.config.OPENWEATHER_API_KEY;
    if (!apiKey) throw new WeatherApiKeyMissingError();

    const cacheKey = `geocode:${q.toLowerCase()}:${limit}`;
    const cached = await this.redis.readJson<GeoCityHit[]>(cacheKey);
    if (cached) return cached;
    ...
    await this.redis.writeJson(cacheKey, hits, GEOCODE_TTL_SECONDS);
    return hits;
  }
}
